...
export default function (state = initialState, action) {
  ...
  switch (action.type) {
    ...
    case actionTypes.UPDATING_TEAM_JOB_FIELD:
      return _.extend({}, state, {
        jobFieldsSaving: state.jobFieldsSaving.concat(action.jobUuid),
      });

    case actionTypes.UPDATED_TEAM_JOB_FIELD:
      jobSavingIdx = state.jobFieldsSaving.indexOf(action.jobUuid);
      ...
      return _.extend({}, state, {
        ...
        jobFieldsShowSuccess: state.jobFieldsShowSuccess.concat(action.jobUuid),
      });

    case actionTypes.HIDE_TEAM_JOB_FIELD_SUCCESS:
      jobSuccessIdx = state.jobFieldsShowSuccess.indexOf(action.jobUuid);
